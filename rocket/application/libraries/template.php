 ...
 * @author     Bonfire Dev Team
 * @version    3.0
 * @link       http://rocketphp.com/docs/guides/views.html
 *
 */
class Template
{
	 ...
	 * no parsing. Used by the r_yield() and block()
	 *
	 * @access public
	 * @static
	 *
	...
	public function __construct()
	{
		...
		if(self::$ci->uri->segment(1) == 'admin')
	...
	public static function render($layout=NULL)
	{
		...
		{			
			self::$current_view =  self::$ci->router->class . '/'. self::$admin_folder . self::$ci->router->method;
		}
	 ...
	 */
	public static function r_yield()
	{
		$output = '';
	...
	public static function set_block($block_name='', $view_name='')
	{
		if (!empty($block_name))
		{
			self::$blocks[$block_name] = self::$admin_folder.$view_name;
		}
	...
	public static function set_view($view=NULL)
	{
		...
		self::$current_view = self::$admin_folder.$view;
	}//end set_view()

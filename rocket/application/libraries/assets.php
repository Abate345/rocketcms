 ...
 *
 * @package   RocketPHP
 * @author    RocketPHP Dev Team
 * @copyright Copyright (c) 2012 - 2013, RocketPHP Dev Team
 ...
 * @category   Libraries
 * @author     RocketPHP Dev Team
 * @link       http://guides.rocketphp.com/core/unit_test.html
 * @version    3.0
 *
 */
class Assets
{
	...
	public static function css($style=null, $media='screen', $bypass_inheritance=FALSE)
	{
		...
		$styles 	= array_merge( $styles,  self::find_files(self::$module_styles, 'css', $bypass_inheritance) );
	...
	public static function module_js($list=FALSE)
	{
		...
		// to the assets.
		$combine = self::$ci->config->item('assets.js_combine');
		if( $combine ){
		...
		foreach ($files as $file)
		{
			...
			if (!empty($module))
			{
				...
				if (!empty($path))
				{
					...
					$new_files[] = self::get_file_array($site_path, preg_replace("#{$file}{$type}#", '', $path), $file, $type, empty($media) ? '' : $media);
				}
	...
	private function get_file_array($site_path='', $path='', $file='', $type='', $media='')
	{
		...
		$file_name = $path.$file;
		if( $site_path )
			$file_name = preg_replace("#$site_path#",'',$file_name);

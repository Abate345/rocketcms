 ...
 * @category   Helpers
 * @author     Bonfire Dev Team
 * @link       http://guides.rocketphp.com/helpers/application_helpers.html
 *
 */
...
if ( ! function_exists('gravatar_link'))
{
	...
	function gravatar_link($email=NULL, $size=48, $alt='', $title='', $class=NULL, $id=NULL)
	{
		...
		$default_image = "http://ineedplan.com/assets/images/user.png";
		...
		return '<img src="'. $avatarURL .'" width="'.	$size .'" height="'. $size . '" title="'. $title .'" ' . $class . $id. ' />';
	}
}
...
if ( ! function_exists('module_urls'))
{
	/**
	 * Returns the 'module_config' array from a modules config/config.php
	 * file. The 'module_config' contains more information about a module,
	 ...
	 * @return array An array of config settings, or an empty array if empty/not found.
	 */
	function module_urls($module_name=null)
	{
		$urls_file = module_file_path($module_name, 'config', 'urls.php');
...
if ( ! function_exists('module_permissions'))
{
	 ...
	 * @return array An array of config settings, or an empty array if empty/not found.
	 */
	function module_permissions($module_name=null)
	{
		$permission_file = module_file_path($module_name, 'config', 'permissions.php');
...
if ( !function_exists('h') )
{
     ...
     *	Saving item history.
     *
     *	@param	$project_id				INT			- project ID, for request localization
     *	@param	$model					VARCHAR		- model name
     *	@param	$item					VARCHAR		- item name
     *	@param	$item_id				INT			- item ID
     *	@param	$user_id				INT			- action owner ID
     *	@param	$action					VARCHAR		- create/edit/delete
     *	@param	$var1, $var2, $var3 	VARCHARs	- use it if need to save extra data, here can be anything you may need
     *	@param	$time 					INT			- time when action start, mostly blank as fill be filled with time()
     *	@return 									- always void
     */
    function h( $project_id, $model, $item, $item_id, $user_id, $action, $var1 = false, $var2 = false, $var3 = false, $time = false ){
        $ci = &get_instance();
		$ci->load->model('history/history_model', null, true);
		
		if( !$time ) $time = time();
		
		$ci->history_model->insert(array(
...
{
    /**
     *	Getting item history. Can get per user, + item, +item_id, +action or any other combination up to blank
     *
     ...
     *	@param	$item_id				INT			- item ID
     *	@param	$action					VARCHAR		- create/edit/delete
     *	@return 									- history item(s)
     */
    function gh( $project_id = false, $user_id = false, $item = false, $item_id = false, $action = false ){
        $ci = &get_instance();
		$ci->load->model('history/history_model', null, true);
		$where = array();
		...
		if( $where ) 		$ci->history_model->where( $where );
		
		$h = $ci->history_model->find_all();
		
		if( @ count( $h ) == 1 ) return $h[0];
...
{
    /**
     *	Getting history message
     *
     *	@param	$h			INT		- project ID
     *	@param	$template	INT		- if model default message template is not good in current situation you can provide own
     *	@return 			VARCHAR	- message
     *	
     *	template ex. 				- {username} create project on {time}
     *	if you need option that is not here create your own, don't be lazy =)
     */
    function hm( $h, $template = false ){
		$ci = &get_instance();
		
		$user = $ci->load->_ci_cached_vars['current_user'];
		
		if( !$template and $h->model ){
			$ci->load->model("{$h->model}/{$h->model}_model", null, true);
			$template = $ci->{"{$h->model}_model"}->{"t_".$h->action};
		}
		
		$message = @preg_replace( "/\{username\}/i", $user->username, $template );
		$message = @preg_replace( "/\{email\}/i", sys_date( $h->time ), $message );
		$message = @preg_replace( "/\{date\}/i", sys_time( $h->time ), $message );
		$message = @preg_replace( "/\{time\}/i", sys_datetime( $h->time ), $message );
		$message = @preg_replace( "/\{datetime\}/i", $user->email, $message );
		
		@preg_match_all( "/\{([a-z0-9]+)\}/i", $message, $matches );
		if( @ count( $matches[1] ) and $h->model and $h->item_id ){
			$ci->load->model("{$h->model}/{$h->model}_model", null, true);
			$obj = $ci->{"{$h->model}_model"}->find( $h->item_id );
			foreach( $matches[1] as $match )
				$message = @preg_replace( "/\{{$match}\}/i", $obj->{$match}, $message );
		}
		return $message;
    }
}
...
if ( !function_exists('sys_time') )
{
	function sys_time( $time = false ){
		if( !$time ) $time = time();
...
{
	function sys_datetime( $time = false ){
		if( !$time ) $time = time();

...
class Users extends Front_Controller
{
	...
	public function login()
	{
		...
		if ($this->auth->is_logged_in() === FALSE)
		{
			if ($this->input->post('submit'))
			{
			...
			}elseif(@$_POST['check_login']){
				if($_POST['email'] && $_POST['password']){
					$user = $this->user_model->find_by("email", xss_clean($_POST['email']));
					if(count($user)){
						if( do_hash($user->salt . xss_clean($_POST['password'])) == $user->password_hash )
							echo json_encode( array( "success" => "true" ) );
						else{
							echo json_encode( array( "error" => "Username/Password incorrect" ) );
						}
					}else{
						echo json_encode( array( "error" => "Username/Password incorrect" ) );
					}
				...
				die();
			}elseif(@$_POST['check_already_login']){
	...
	public function register()
	{
		...
		{
			// Validate input
			$this->form_validation->set_rules('full_name', 'Full Name', 'required|trim|strip_tags|xss_clean');
			$this->form_validation->set_rules('username', 'Username', 'required|trim|strip_tags|xss_clean');
			$this->form_validation->set_rules('email', 'Email', 'required|trim|strip_tags|valid_email|max_length[120]|unique[users.email]|xss_clean');
			$this->form_validation->set_rules('password', 'Password', 'required|trim|strip_tags|min_length[8]|max_length[120]|valid_password');
			$this->form_validation->set_rules('pass_confirm', 'Password (repeat)', 'required|trim|strip_tags|matches[password]');
			$this->form_validation->set_rules('plan', 'Plan', 'required|trim|strip_tags|xss_clean');
			$this->form_validation->set_rules('timezone', 'Timezone', 'required|trim|strip_tags|max_length[6]|xss_clean');
			...
			$this->form_validation->set_rules('plan', 'Plan', 'required|trim|strip_tags|xss_clean');
			$this->form_validation->set_rules('timezone', 'Timezone', 'required|trim|strip_tags|max_length[6]|xss_clean');
			$this->form_validation->set_rules('terms', 'Agreement', 'required|trim|strip_tags|xss_clean');
			if ($this->form_validation->run($this) !== FALSE){
				echo json_encode( array( "success" => "true" ) );
				die();
			}else{
	...
	private function save_user($id=0, $meta_fields=array())
	{
		...
		if($this->current_user->email != $_POST['email'] || $_POST['email'] == '')
			$this->form_validation->set_rules('email', 'Email', 'required|trim|strip_tags|valid_email|max_length[120]|unique[users.email]|xss_clean');

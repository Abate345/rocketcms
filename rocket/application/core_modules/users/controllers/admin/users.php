...
class Users extends Admin_Controller
{
	...
	public function index($offset=0)
	{
		...
		$this->pager['base_url'] = site_url(SITE_AREA .'/users/index');
		...
		if ($this->input->post('submit'))
		{
			...
			if ($id = $this->save_user('insert', NULL, $meta_fields))
			{
				...
				*/
				...
				Template::redirect(SITE_AREA .'/users');
			}
		}
	...
	private function _delete($id)
	{
		...
		if (isset($user) && $user->id != $this->current_user->id)
		{
	...
	public function access_logs($limit=15)
	{
		...
		return $this->load->view('admin/access_logs', array('access_logs' => $logs), TRUE);
	...
	private function save_user($type='insert', $id=0, $meta_fields=array(), $cur_role_name = '')
	{
		...
		$this->form_validation->set_rules('display_name', lang('bf_display_name'), 'required|trim|strip_tags|max_length[255]|xss_clean');
		...
		$this->form_validation->set_rules('role_id', lang('us_role'), 'required|trim|strip_tags|max_length[4]|is_numeric|xss_clean');
	...
	private function user_status($user_id = false, $status = 1, $supress_email = 0)
	{
		...
		if ($user_id !== false && $user_id != -1)
		{
			...
			if ($result)
			{
				...
				{
					// Now send the email
					$this->load->library('emailer/emailer_lib');
					...
					if ($this->emailer_lib->send($data))
					{
						$message = lang('us_active_email_sent');
					}
					else
					{
						$message=lang('us_err_no_email'). $this->emailer_lib->errors;
					}
				}

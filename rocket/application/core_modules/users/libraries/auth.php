...
class Auth
{
	 ...
	 */
	public function restrict($permission=NULL, $level=NULL, $uri=NULL )
	{
		// If user isn't logged in, don't need to check permissions
		if ($this->is_logged_in() === FALSE)
		{
		...
		// Check to see if the user has the proper permissions
		if (!empty($permission) && !$this->has_permission($permission, false, $level))
		{
	 ...
	 */
	public function has_permission($permission = NULL, $role_id=NULL, $level = NULL)
	{
		/*if(empty($level))
			echo $permission."---".$level."<br />";*/
		
		
		if (empty($permission) || empty($level))
		{
			/*if(empty($level)){
				echo "Find and add level to permission. Usually it is located in controller of section you are using.";
				die();
			}	*/
			return FALSE;
		}
		...
		if (isset($perms->$permission) && $perms->$permission >= $level)
		{
			return TRUE;
		}
	...
	public function permission_exists($permission=NULL)
	{
		...
		if (!isset($this->all_perms)) {
			...
			if(count($perms) AND is_array($perms)){
	...
	public function load_permissions($role_id=NULL)
	{
		...
		$perms_all = $this->ci->permission_model->find_all();
	
		$perms = array();
		if(count($perms_all) AND is_array($perms_all)){
			foreach($perms_all as $key => $perm_details)
			{
				if( (int) $perm_details->status > 0)
		...
		if(count($perms) AND is_array($perms)){
			if (is_array($role_perms))
			{
				...
				{
					$curr_role_permission = $this->ci->permission_model->find_all_by('permission_id', $permission->permission_id);
					$this->perms[strtolower($perms[$permission->permission_id])] = $curr_role_permission[0]->status;
				}
			}
		}
...
if (!function_exists('has_permission'))
{
	 ...
	 * @return bool TRUE/FALSE
	 */
	function has_permission($permission=NULL, $level = NULL)
	{
		$ci =& get_instance();

		if (class_exists('Auth'))
		{
			return $ci->auth->has_permission($permission, NULL, $level);
		}

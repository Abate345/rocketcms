...
class Role_model extends BF_Model
{
	...
	public function get_role_permissions(&$role)
	{
		...
		// Permissions
		if(count($permissions) AND is_array($permissions)){
		...
		if (is_array($role_permissions) && count($role_permissions))
		{
			foreach($role_permissions as $key => $permission)
			{
				...
				$permission_tmp = $this->permission_model->find_all_by('permission_id',$permission->permission_id);
				$permission_new_array[$permission_tmp[0]->name] = $permission_tmp[0]->status;
			}
		}

		$role->role_permissions = $permission_array;
		$role->new_role_permissions = $permission_new_array;

 ...
 *
 */
class Emailer extends Admin_Controller
{
	...
	public function template()
	{
		if ($this->input->post('submit'))
		{
			...
			redirect(SITE_AREA .'/emailer/template');
		}
	...
	public function test()
	{
		...
		$this->load->library('emailer_lib');
		$this->emailer_lib->enable_debug(TRUE);
		...
		$results = $this->emailer_lib->send($data, FALSE);
	...
	public function queue()
	{
		...
		elseif (isset($_POST['action_force_process']))
		{
			...
			ob_start();
			$this->emailer_lib->process_queue();
			ob_end_clean();
		}
		elseif (isset($_POST['action_insert_test']))
		{
			...
			$this->emailer_lib->send($data, TRUE);
		}
		...
		$this->pager['base_url'] = site_url(SITE_AREA .'/emailer/queue');
			...
			else
			{
				...
				if (is_array($checked) && count($checked))
				{
					...
					foreach ($checked as $user_id)
					{
						...
						if ($user != NULL){
							$data['to'] = $user->email;
							$result = $this->emailer_lib->send($data,TRUE);
					...
					if ($result)
					{
						...
						Template::redirect(SITE_AREA . '/emailer/queue');
					}
					else
					{

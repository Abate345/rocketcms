...
class Translate extends Admin_Controller
{
	...
	public function edit($trans_lang='', $lang_file='')
	{
		...
		if(!$trans_lang)
			$trans_lang = $this->uri->segment(4);
		
		if(!$lang_file)
			$lang_file = $this->uri->segment(5);
		...
		if ($lang_file && $this->input->post('submit'))
		{
			if (save_lang_file($lang_file, $trans_lang, $_POST['lang']))
			{
				...
				redirect(SITE_AREA .'/translate/index/'. $trans_lang);
			}
			else
			{
	...
	public function do_export($language=NULL, $include_core=FALSE, $include_custom=FALSE)
	{
		...
		$this->zip->download('rocketphp_'. $language .'_files.zip');
        die();
